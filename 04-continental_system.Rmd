...
\chapter[A continental system for forecasting bird migration]{A continental system for\\forecasting bird migration}
...
```{r forecast, out.width = "100%", message=F, echo=F, fig.align='center'}
pages <- list.files(file.path("pdf_chapters/forecast"),pattern="forecast_crop_Part",full.names = T)

knitr::include_graphics(pages)  
# if (knitr::is_latex_output()) cat('\\pagebreak')
```

```{r forecast-supp, out.width = "100%", message=F, echo=F}
pages <- list.files(file.path("pdf_chapters/forecast"),pattern="forecast_supp_crop_Part",full.names = T)
